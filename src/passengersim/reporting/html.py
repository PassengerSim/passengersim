...
def to_html(
    ...
    *,
    ...
    add_timestamp : bool, optional
        ...
        overwrite the same file with each run, or if you are handling the
        timestamping yourself.
    ...
    if cfg.outputs.html.bookings_by_timeframe:
        try:
            ...
            for c in carriers:
                ...
                rpt.add_figure(
                    summary.fig_segmentation_by_timeframe("bookings", by_carrier=c, by_class=True, also_df=True)
                )

    if cfg.outputs.html.segmentation_by_timeframe_table:
        if summary.segmentation_by_timeframe is not None:
            rpt.add_table(
