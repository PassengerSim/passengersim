...
class QPathForecast(RmAction):
    """
    Implied Q path-level demand forecasting tool.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # This RmAction requires that the carrier have a Frat5 curve defined for each market.
        # To ensure that is the case, we also require a default Frat5 curve to be defined,
        # which applies to all markets for the carrier unless a different market-specific
        # curve is defined.
        if cfg is None:
            raise ValueError("Config object must be provided to QPathForecast to validate Frat5 curves.")
        if not carrier:
            raise ValueError("Carrier must be specified for Q path forecasting.")
        if carrier not in cfg.carriers:
            raise ValueError(f"Carrier '{carrier}' not found in config for Q path forecasting.")
        carrier_cfg = cfg.carriers[carrier]
        if not carrier_cfg.frat5:
            raise ValueError(f"Carrier '{carrier}' must have a Frat5 curve defined for Q path forecasting.")
        if not carrier_cfg.store_q_history:
            raise ValueError(f"Carrier '{carrier}' must have 'store_q_history' set to True for Q path forecasting.")

        # This RmAction also generally depends on the max_cap parameter being set
        # to some reasonable finite value, to prevent extreme sellup factors from causing
        # excessively high weighting.  The max_cap also needs to be set in two places,
        # first here in the RmAction, and second in the carrier Frat5 config for use
        # by the Q demand inflator that runs inside the simulation (separate from this
        # RmAction). From a purely computational standpoint, it is not strictly necessary
        # that these two max_cap values be the same, but for consistency and to avoid
        # confusion it is enforced here that they be the same.
        if max_cap <= 0.0:
            raise ValueError("max_cap must be set to a value greater than zero for Q path forecasting.")
        if carrier_cfg.frat5 not in cfg.frat5_curves:
            # if the frat5 curve is not found, it might be because it is a standard
            # curve that was not yet loaded, so we will check those also...
            try:
                cfg._load_std_frat5(carrier_cfg.frat5)
            except KeyError:
                raise ValueError(
                    f"Frat5 curve '{carrier_cfg.frat5}' for carrier '{carrier}' "
                    f"not found in config for Q path forecasting."
                ) from None
        carrier_frat5 = cfg.frat5_curves[carrier_cfg.frat5]
        if carrier_frat5.max_cap != max_cap:
            raise ValueError(
                f"max_cap value {max_cap} in QPathForecast does not match "
                f"max_cap value {carrier_frat5.max_cap} in Frat5 config for '{carrier_cfg.frat5}'."
            )
    ...
    def run(self, sim: Simulation, days_prior: int):
        ...
        for thing in things:
            # Get the Frat5 curve for this specific market (or for the leg, as applicable)
            f5 = carrier_obj.get_frat5_mkt(thing.orig, thing.dest)
            if f5 is None:
                raise ValueError(f"Frat5 curve not found for hybrid forecasting for market {thing.orig}-{thing.dest}")
            ...
            if dcp_index == 0:
                ...
                if self.variance_is_ratio_of_mean:
                    raise NotImplementedError(
                        "variance_is_ratio_of_mean is not yet implemented for Q/hybrid forecasting."
                    )
...
class QLegForecast(QPathForecast):
    """
    Leg-level Q-demand forecasting tool.
    """
