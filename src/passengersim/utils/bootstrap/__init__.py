...
class BootstrapHtml:
        ...
        *,
        ...
        if title is None:
            try:
                ...
                if hasattr(fig, "attrs") and "title" in fig.attrs and (t := fig.attrs["title"]):
                    title = t
                else:
    ...
    def add_extra(self, obj, *args):
        ...
        - A string starting with "> " will create a new preformatted text block.
        ...
        for item in args:
            ...
            if isinstance(item, str) and item.startswith("## "):
                ...
                continue
            if isinstance(item, str) and item.startswith("> "):
                self.current_section.elem("pre", text=item[2:])
                continue
