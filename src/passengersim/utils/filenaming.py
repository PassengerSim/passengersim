...
def timestamp_now() -> str:
    ...
    return _format_timestamp(time.time())
...
def filename_with_timestamp(
    ...
    *,
    ...
    timestamp : str, float, time.struct_time, datetime.datetime, or None
    ...
    if not isinstance(timestamp, str):
        timestamp: str = _format_timestamp(timestamp)
    ...
    if match:
        # Older versions of this code would change the timestamp to the current time...
        # filename_str = re.sub(pattern, f".{timestamp}{suffix}", filename_str)
        # filename = pathlib.Path(filename_str)
        # ... but let's not do that so we can sync timestamps across files.
        pass
    else:
    ...
    while filename.exists() and timestamp != "no-timestamp":
        ...
        timestamp = f"{t0}-{t1}"
        new_filename = filename.with_stem(filename.stem.replace(old_timestamp, timestamp))
        if new_filename == filename:
            raise ValueError(f"oops, filename already exists: {filename}")
        else:
