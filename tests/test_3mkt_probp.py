...
def config() -> Config:
    ...
    cfg.carriers.AL1.rm_system_options = {"name": "P", "bid_price_vector": False}
    cfg.carriers.AL2.rm_system_options = {"name": "P", "bid_price_vector": False}
...
def test_3mkt_probp_table_single_process(summary, dataframe_regression, table_name: str):
...
@pytest.mark.parametrize("table_name", TABLES)
def test_3mkt_probp_table_multi_process(summary_mp, dataframe_regression, table_name: str):
...
def test_3mkt_probp_fig_segmentation_by_timeframe(summary, dataframe_regression, by_carrier, by_class, measure):
    assert isinstance(summary, SimulationTables)
    assert isinstance(
...
def test_3mkt_probp_fig_carrier_load_factors(summary, dataframe_regression):
    assert isinstance(summary, SimulationTables)
...
def test_3mkt_probp_fig_carrier_mileage(summary, dataframe_regression):
    assert isinstance(summary, SimulationTables)
...
def test_3mkt_probp_fig_carrier_revenues(summary, dataframe_regression):
    assert isinstance(summary, SimulationTables)
...
def test_3mkt_probp_fig_carrier_yields(summary, dataframe_regression):
    assert isinstance(summary, SimulationTables)
...
def test_3mkt_probp_fig_fare_class_mix(summary, dataframe_regression):
    assert isinstance(summary, SimulationTables)
...
def test_3mkt_probp_fig_od_fare_class_mix(summary, dataframe_regression):
    assert isinstance(summary, SimulationTables)
...
def test_3mkt_probp_fig_path_forecasts(summary, dataframe_regression, of: Literal["mu", "sigma"]):
    assert isinstance(summary, SimulationTables)
