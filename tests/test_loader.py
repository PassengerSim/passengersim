...
from passengersim.rm import list_registered_rm_systems
...
def test_carriers_std_rm():
    ...
    assert loaded.carriers["AL2"].frat5 == "curve_C"
    assert not loaded.rm_systems.keys()
    assert "E" in list_registered_rm_systems()
...
def test_carriers_std_frat5():
    ...
    content = yaml.safe_load(io.StringIO(demo))
    loaded = Config.model_validate(content)
    assert loaded.carriers["AL1"].frat5 == ""  # missing curve (not used)
    assert loaded.carriers["AL2"].frat5 == "flat_curve"  # defined on carrier
    assert loaded.frat5_curves.keys() == {"flat_curve"}
